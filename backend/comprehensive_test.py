...
async def test_agent_import():
    ...
    try:
        ...
        print(f"   ✅ invoke_agent function exists with params: {list(sig.parameters.keys())}")
...
async def test_key_format_validation():
    ...
    if api_key:
        ...
        print(f"   ✅ Key is fake (test key): {is_fake}")
        print(f"   ✅ Key is valid OpenRouter format: {is_valid_opensource}")
...
async def test_client_configuration():
    ...
    try:
        ...
        print(f"   ❌ Error testing client configuration: {e}")
        return False
...
async def test_mock_fallback():
    ...
    if api_key and api_key.startswith("fake-"):
        print("   ✅ Using fake key, mock fallback should be activated")
        print("   ✅ New logic will detect fake key and use mock response")
        print("   ✅ This prevents 401 errors from being raised unnecessarily")
        return True
    elif api_key and api_key.startswith("sk-or-"):
        print("   ✅ Using real key, API call should proceed normally")
        return True
    else:
        print("   ⚠️  Unknown key format, system will attempt call with fallback")
        return True
