...
def test_fake_key_detection():
    ...
    try:
        ...
        if 'fake-' in source and 'startswith' in source:
            print("[OK] Fake key detection logic found in call_openai_agent")
        else:
            print("[FAIL] Fake key detection logic NOT found in call_openai_agent")

        if 'test' in source and 'in api_key.lower()' in source:
            print("[OK] Fake key detection for 'test' patterns found")
        else:
            print("[FAIL] Fake key detection for 'test' patterns NOT found")

        # Check for early return mechanism
        if 'return mock_message' in source and 'Fake/Invalid API key detected' in source:
            print("[OK] Early return mechanism found")
        else:
            print("[FAIL] Early return mechanism NOT found")

        return True

    except ImportError as e:
        print(f"[FAIL] Could not import agent module: {e}")
        return False
    except Exception as e:
        print(f"[FAIL] Error testing agent module: {e}")
        return False

def test_invoke_agent():
    """Test the invoke_agent function as well"""
    print("")
    print("=== Testing invoke_agent Function ===")
    ...
    try:
        ...
        if 'fake-' in source and 'startswith' in source:
            print("[OK] Fake key detection logic found in invoke_agent")
        else:
            print("[FAIL] Fake key detection logic NOT found in invoke_agent")

        if 'Fake API key detected' in source:
            print("[OK] Fake key message found in invoke_agent")
        else:
            print("[FAIL] Fake key message NOT found in invoke_agent")
...
def main():
    ...
    if test1_result and test2_result:
        ...
        print("")
        print("[FAIL] IMPLEMENTATION INCOMPLETE")
