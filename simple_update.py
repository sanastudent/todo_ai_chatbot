#!/usr/bin/env python3
"""
Simple script to replace the entire invoke_agent function
"""
...
# Define the complete new function
new_function = '''async def invoke_agent(
    user_id: str,
    conversation_id: str,
    user_message: str,
    ...
    """
    Invoke the AI agent with conversation history and tools
    Uses OpenRouter API if available, otherwise falls back to mock responses
    """
    try:
...
for i, line in enumerate(lines):
    if line.strip().startswith('async def invoke_agent('):
        new_lines.append(new_function)
        # Skip until we find the next function or end of file
        skip = True
    elif skip:
